...
| **🧠 Use memory** | Prepends a review directive to every investigation you launch, telling the agent to consult the named context-memory file — searching it for the task's specific entities rather than reading it in full — and apply its ground truth *before* rendering a verdict. | A **per-launch alternative to the auto-loaded user-memory trigger rule** above. Handy when the trigger isn't set up — e.g. a fresh worktree that never restored user memory. Both can coexist. |
