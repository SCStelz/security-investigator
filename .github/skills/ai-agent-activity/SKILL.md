...
description: 'Report/investigate RUNTIME ACTIVITY of AI agents (Agent 365 / Copilot Studio / M365 Copilot / Work IQ) — agents used, tools/connectors, channels, tokens, prompt/reply content, and Prompt Shield jailbreak/XPIA verdicts. Triggers: "agent activity", "AI agent usage", "who is using agents", "agent runtime", "agent telemetry", "agent tool usage", "agent session", "jailbreak activity", "prompt injection activity", "Agent 365 activity", "UnifiedAgentObservability", "CloudAppEvents agents", "CopilotActivity". Auto-detects available data planes — UnifiedAgentObservability (Data Lake), CloudAppEvents (Defender), CopilotActivity (governance/fail-close) — and uses what is present. Supports tenant-wide, single-agent, and single-user scopes. Runtime companion to the config-focused ai-agent-posture skill.'
