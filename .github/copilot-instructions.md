...
| 🤖 GenAI | **ai-agent-activity** | AI agent runtime activity (agents used, tools invoked, users, channels, jailbreak/XPIA safety) — auto-detects UnifiedAgentObservability (Data Lake) or CloudAppEvents (Defender) | "agent activity", "agent activity report", "AI agent usage", "who is using agents", "agent runtime", "agent tool usage", "agent session", "jailbreak activity", "Agent 365 activity", "UnifiedAgentObservability" |
